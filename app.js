...
class Startup {
    ...
    riceviIncentivo(incentivo) {
        ...
        if (this.incentiviRicevuti.some((i) => i.codiceIdentificativo === incentivo.codiceIdentificativo)) {
            // esegui l'azione se l'incentivo è stato approvato
            console.log(`La startup ${this.nome} ha ricevuto l'incentivo ${incentivo.codiceIdentificativo} dal valore di ${incentivo.valore}.`);
            console.log(`Gli incentivi della startup ${this.nome} sono:`, this.incentiviRicevuti);
        }
        else {
            console.log(`La startup ${this.nome} non ha ricevuto l'incentivo ${incentivo.codiceIdentificativo}.`);
        }
    }
}
class Incentivo {
    ...
    assegnaAStartup(startup) {
        ...
        if (this.criteriEleggibilita.includes(startup.settoreFocus)) {
            ...
            startup.incentiviRicevuti.push(this),
                console.log(`L'incentivo ${this.codiceIdentificativo} è stato assegnato a ${startup.nome}.`);
        }
        else {
            console.log(`La Startup ${startup.nome} non soddisfa i criteri di eleggibilità dell'incentivo ${this.codiceIdentificativo}.`);
        }
    }
}
class Cittadino {
    ...
    partecipaAttività(startup) {
        ...
        console.log(`${startup.nome} ha i cittadini partecipanti:`, startup.cittadiniPartecipanti);
    }
}
// creazione startup
const startup1 = new Startup("FitTech", "App per il fitness", "Sviluppiamo app per il fitness che aiutano gli utenti a monitorare i loro progressi", ["App di monitoraggio fitness", "Servizi di consulenza fitness"]);
const startup2 = new Startup("WearSport", "Tecnologie wearable", "Innoviamo il mondo dello sport con dispositivi wearable che monitorano le prestazioni", ["Smartwatch per atleti", "Abbigliamento sportivo intelligente"]);
// creazione incentivo
const incentivo1 = new Incentivo("INC001", "Incentivo per lo sviluppo app fitness", 10000, ["App per il fitness", "Sviluppo di una community online per sportivi"]);
const incentivo2 = new Incentivo("INC002", "Incentivo per le tecnologie wearable", 15000, ["Tecnologie wearable", "Smartwatch per atleti", "Abbigliamento intelligente"]);
const incentivo3 = new Incentivo("INC003", "Incentivo per l'AI nella salute", 7500, [
    "App per il fitness",
